  ...
  <div class="comments-section">
    ...
    <div v-for="comment in comments" :key="comment.id" class="comment">
      ...
      <div class="comment-actions">
        ...
        <div class="reaction-buttons"  @mouseenter="handleMouseEnter(comment.id)" @mouseleave="handleMouseLeave">
          <button class="like-button" @click="reactToComment(comment.id, 'THUMBS_UP')">
            {{ getTotalReactions(comment.reactions) }} 👍 
          </button>
          <div class="all-reactions" v-if="hoveredCommentId === comment.id"  >
            <span v-for="(count, emoji) in comment.reactions" :key="emoji" @click="reactToComment(comment.id, emoji)" >
              {{ getEmoji(emoji) }} {{ count }}
            </span>
          </div>
          <span
...
export default {
  ...
  methods: {
    async fetchComments() {
      try {
        ...
        if (response.status === 200) {
          this.comments = response.data.sort((a, b) => new Date(b.time) - new Date(a.time));
          await Promise.all(
    ...
    getTotalReactions(reactions) {
      // Kiểm tra reactions có phải là đối tượng hợp lệ không
      if (reactions && typeof reactions === 'object') {
        // Tổng số lượng react
        return Object.values(reactions).reduce((sum, count) => sum + count, 0);
      }
      // Trả về 0 nếu reactions không hợp lệ
      return 0;
    },
